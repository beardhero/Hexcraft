...
export class DemoRoom extends Room {
  ...
  onCreate (options: any) {
    ...
    this.onMessage("TypeMessage", (client, message) => {
      console.log("server is broadcasting chat message: "+message);
      this.broadcast("TypeMessage", {msgType: message.msgType, contents: message.contents});
    });

    this.onMessage("*", (client, type, message) => {
      console.log(`received unhandled message "${type}" from ${client.sessionId}:`, message);
    });
  }
